...
初稿由 AI 生成，随后经 AI 辅助审查、评估流程修复和自动化测试整理。代码与数据为独立学习示例，不包含历史课程作业或商业项目的原始源码。
...
原型先在全量数据上构造特征，梯度提升预测后续测试日时读到了测试期真实流量，而 ARIMA 使用固定起点多步预测。两者评估条件不一致。本版先切分训练数据，再为梯度提升递归生成未来特征，统一比较口径；同时移除全局警告屏蔽，以及安装 Torch 就自动启用的未验证 LSTM 分支。
